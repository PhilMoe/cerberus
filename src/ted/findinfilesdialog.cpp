...
//----------------------------------------------------------------------------------------------------------------------
//  FindDialog: PUBLIC MEMBER FUNCTIONS
//----------------------------------------------------------------------------------------------------------------------
...
#else
    QStringList filters = tfilters.split(" ", Qt::SkipEmptyParts);
#endif
    QStack<QString> todo;
    todo.push(dir);
...
void FindInFilesDialog::browseForDir()
{
    ...
    QString path = fixPath(QFileDialog::getExistingDirectory(
                               this, "Select directory", dir, QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks));
    if(path.isEmpty())
        return;
