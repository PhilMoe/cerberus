...
//----------------------------------------------------------------------------------------------------------------------
// Operating specific static functions
//----------------------------------------------------------------------------------------------------------------------
// NOTE: Windows code is for killing a process, while the Linux/Mac-OS code is to copy the command line into args and
//       argv. Some macros are defined here for closing file handles, pipes, and standard file streams.
#ifdef Q_OS_WIN
...
//----------------------------------------------------------------------------------------------------------------------
// Copy the command line to args and argv
//----------------------------------------------------------------------------------------------------------------------
// NOTE: C style memory allocation is commented out here just in case there are issues with the C++ style memory
//       allocations
static char **makeargv(const char *cmd)
{
    ...
    // If args and argv are assigned, then free the memory.
    if(args)
        //free(args);
        delete args;
    if(argv)
    ...
    args = new char[strlen(cmd) + 1];
    strncpy(args, cmd, strlen(cmd) + 1);
...
// Start a new process by passing the full command line.
bool Process::start(const QString &cmd)
{
    ...
    // NOTE: vfork() with modern *nix kernels should no longer be needed with them implementing a copy-on-write. But if
    // any oddities happen, then switch back to using vfork().
    _pid = fork();
...
// Clean up a process when finished.
Process::~Process()
{
    ...
    if(_procwaiter->wait(10000))
        delete _procwaiter;
    else
        cdebug("Timeout waiting for process to finish");

    if(_linereaders[0]->wait(1000))
        delete _linereaders[0];
    else
        cdebug("Timeout waiting for stdout reader to finish");
    if(_linereaders[1]->wait(1000))
        delete _linereaders[1];
    else
        cdebug("Timeout waiting for stderr reader to finish");
...
// Adds a delay if the process is still running.
bool Process::wait()
{
    if(_state != RUNNING)
...
// Forces the termination of a process
bool Process::kill()
{
    qDebug() << "Killing proc";
...
// If a line was written successfully, the return a true value.
...
///----------------------------------------------------------------------------------------------------------------------
//  ProcWaiter: IMPLEMENTATION
//----------------------------------------------------------------------------------------------------------------------
ProcWaiter::ProcWaiter(pid_t pid) : _pid(pid) {}

///----------------------------------------------------------------------------------------------------------------------
//  ProcWaiter: PUBLIC MEMBER FUNCTIONS
//----------------------------------------------------------------------------------------------------------------------
void ProcWaiter::run()
{
#ifdef Q_OS_WIN
...
bool LineReader::waitLineAvailable(int millis)
{
    if(!_avail && !_eof) {
        if(!wait(millis))
            cdebug("Timeout waiting for process output");
    }
    return _avail;
}
...
///----------------------------------------------------------------------------------------------------------------------
//  LineReader: PRIVATE MEMBER FUNCTIONS
//----------------------------------------------------------------------------------------------------------------------
void LineReader::run()
{
    _buf.clear();
...
#ifdef Q_OS_WIN
    DWORD n = 0;
    if(ReadFile(_fd, &c, 1, &n, nullptr) && n == 1)
        return c;
#else
    if(read(_fd, &c, 1) == 1)
        return c;
